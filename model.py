...
def softmax_overflow_demo(large_value):
    ...
    result_val = float(exp_arr[0])
    overflowed = np.isinf(result_val)
    
    return {
        'naive_exp': result_val,
...
def stable_softmax_1d(logits):
    ...
    exp_logits = array_exp(logits - max_along_axis(logits,axis=0))
    return exp_logits/sum_all(exp_logits)
    pass
...
def stable_softmax_2d_rowwise(logits):
    ...
    #exp_logits = array_exp(logits - max_along_axis(logits,axis=1))
    #return exp_logits/sum_keepdims(exp_logits,axis=1)
    shifted_logits = logits - np.max(logits, axis=1, keepdims=True)
    
    # Exponentiate shifted values
    exp_logits = np.exp(shifted_logits)
    
    # Normalize by row sums (keepdims=True maintains shape (N, 1) for division)
...
def read_text_file(text_blob):
    ...
    if not isinstance(text_blob, str):
        raise TypeError(f"Expected str, got {type(text_blob).__name__}")
    if len(text_blob) == 0:
...
def sample_random_batch_offsets(data_len, block_size, batch_size, rng):
    ...
    max_block = data_len - block_size -1
    if max_block <= 0:
        raise ValueError(
            f"Dataset length ({data_len}) must be strictly greater "
            f"than block_size ({block_size}) to sample a valid batch."
        )
    return rng.integers(0,max_block+1,batch_size)
    pass
...
def stack_x_batch(data, offsets, block_size):
    ...
    return np.array([slice_x_at_offset(data,off,block_size) for off in offsets])
    pass
...
def stack_y_batch(data, offsets, block_size):
    ...
    return np.array([slice_y_at_offset(data, off, block_size) for off in offsets])
    pass

# Step 44 - get_batch
def get_batch(data, block_size, batch_size, rng):
    # TODO: package one training batch (X, Y) of shape (batch_size, block_size) from data using rng.
    offsets = sample_random_batch_offsets(data, block_size, batch_size, rng)
